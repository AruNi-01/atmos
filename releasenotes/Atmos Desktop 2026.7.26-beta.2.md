> **Beta release.** Beta 2 supersedes Beta 1. It carries all Beta 1 content plus the Windows build fix below. Please report any issues before the stable release.

## Changes Since Beta 1

- Fixed the Windows desktop runtime build for Disk Analyzer: identity and allocated-size queries now use stable Win32 handle APIs instead of `CreateFileW` (needs `Win32_Security`) and unstable `windows_by_handle` metadata methods.
