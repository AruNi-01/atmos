> **Beta release.** Beta.16 supersedes beta.15. It carries all beta.15 content plus the CLI, local runtime, and cross-platform hook fixes below. Please report any issues you encounter so we can land them before the stable release.

## Changes Since 1.2.0-beta.15

- **CLI 0.2.0 Alignment** — Bundled the stable Atmos CLI `0.2.0` release and added JSON output support for CLI workflows that need machine-readable responses.
- **Local Runtime Refresh** — Bundled local web runtime `0.2.0-beta.11` and removed the legacy local runtime alias from docs and runtime paths so local setup uses the current command surface consistently.
- **Hermes Cross-Platform Fix** — Guarded Hermes executable permission handling on non-Unix platforms to avoid permission-mode assumptions outside macOS/Linux.

---

> **Beta release.** Beta.15 supersedes beta.14. It carries all beta.14 content plus the fixes below. Please report any issues you encounter so we can land them before the stable release.

## Changes Since 1.2.0-beta.14

- **Diff Annotation Prompt Stash** — Added prompt stashing support for diff annotations so annotation work can retain context and reuse prompt state between workflows.
- **Hermes Prompt Flag Query Syntax** — Added support for Hermes-specific prompt flag query syntax to improve compatibility with runtime command parsing.
- **Generated Asset Ignore Fix** — Adjusted ignore rules so generated GSAP runtime assets are excluded from packaging paths that should remain stable for local runtime builds.
- **Video Scaffold Path Guard** — Prevented node_modules script paths from leaking into video scaffold generation, reducing transient packaging/launch noise.
- **Code Quality Cleanup** — Included a set of follow-up quality fixes from the daily stability pass to stabilize related preview and agent-facing flows.

---
> **Beta release.** Beta.14 supersedes beta.13. It carries all beta.13 content plus the mobile app MVP, GitHub Issue automation, local service management, and the fixes below. Please report any issues you encounter so we can land them before the stable release.

## Changes Since 1.2.0-beta.13

- **Atmos Mobile App MVP** — Added the first cross-platform Atmos mobile app (Expo / iOS + Android) with native workspace terminal UI, mobile settings and relay terminology, refined cache keys and terminal attach handling, and iOS simulator keyboard troubleshooting docs. The mobile app connects through the Atmos relay for on-the-go workspace access.
- **GitHub Issue Automation Judge** — Added end-to-end GitHub Issue automation with a judge workflow that evaluates new issues and applies `atmos-judge-*` labels, plus an implementation workflow that runs when issues are labeled `atmos-judge-approve`. Includes kanban checkbox toggles for filtering automation workspaces ([#128](https://github.com/AruNi-01/atmos/pull/128)).
- **Local Run Service Manager** — Added a local run service manager with per-service controls, loopback URL handling, fullscreen layering for run previews, and clickable note controls in preview selection.
- **Hover Peek Sidebars** — Hover-triggered side panels now let you quickly preview content without navigating away from the current view, with preserved file names in narrow diff sidebars.
- **Local Agent CLI Routing** — Automation runs can now route through a local agent CLI for more flexible execution.
- **Web Bootstrap Performance** — Optimized bootstrap request fan-out for faster initial load times on the web app.
- **Preview Selection Improvements** — Refined the selection picker flow with direct input card display on element select, favicon support, annotation support, and normalized preview titles.
- **Canvas Terminal Stability** — Fixed canvas terminal new-tab lifecycle, isolated terminal layout context state, and resolved interactive terminal agent summary handling.
- **Automation Polish** — Added an automation enable switch, improved run streaming and runner metadata, added Pi OpenClaw and Hermes terminal agents, fixed terminal handoff for continued runs, and simplified automation prompts.
- **Code Quality Refactors** — Addressed daily quality findings across preview toolbar extension mode, selection popover extraction, terminal agent run-config per-agent handling, and commit panel/settings layout with shared basename helpers ([#125](https://github.com/AruNi-01/atmos/pull/125), [#126](https://github.com/AruNi-01/atmos/pull/126), [#127](https://github.com/AruNi-01/atmos/pull/127)).
- **CLI & Runtime Bumps** — Bundled Atmos CLI updated to `0.2.0-beta.9` and local web runtime to `0.2.0-beta.9`.

---

> **Beta release.** Beta.13 supersedes beta.12. It carries all beta.12 content plus the cross-platform desktop build fix below. Please report any issues you encounter so we can land them before the stable release.

## Changes Since 1.2.0-beta.12

- **Desktop Linux Build Fix** — Guarded the Appshots permissions window titlebar styling behind macOS-only compilation so the desktop release workflow can build the Linux target while preserving the overlay titlebar treatment on macOS.

---

> **Beta release.** Beta.12 supersedes beta.11. It carries all beta.11 content plus the remote-access naming cleanup, Appshots polish, global-search additions, automation gating, and sidebar layout fixes below. Please report any issues you encounter so we can land them before the stable release.

## Changes Since 1.2.0-beta.11

- **Tunnel Connector Naming** — Renamed the older Remote Access tunnel surface to **Tunnel Connector** across desktop commands, Rust crates, web settings, docs, release notes, and specs so it is clearly separate from Atmos Computer remote runtime switching.
- **Appshots Workflow Polish** — Added the desktop permissions-window flow, refined Appshot context and composer icon presentation, and compacted history rows so captured screenshots are easier to review and reuse from the web UI.
- **Automations Preview Gating** — Gated the automations management UI behind its experiment flag and fixed terminal-agent resolution from user bin paths, keeping the beta automation surface more predictable during rollout.
- **Global Search Expansion** — Added project overview, Canvas, and per-section Settings entries to global search for quicker navigation into common management and configuration surfaces.
- **Sidebar Layout Stability** — Stabilized left-sidebar resize behavior, first-run two-column collapse sizing, persisted layout defaults, and sidebar layout debug logging to reduce layout jumps around grouped sidebars.

---

> **Beta release.** Beta.11 supersedes beta.10. It carries all beta.10 content plus the cross-app Appshots workflow below and refreshed companion beta bundles. Please report any issues you encounter so we can land them before the stable release.

## Changes Since 1.2.0-beta.10

- **Cross-App Appshots** — Added the first cross-app Appshots capture flow, including desktop capture plumbing, clipboard/thumbnail/history handling, welcome-composer paste support, and a web history UI so screenshots move cleanly from the desktop shell into Atmos conversations and automations ([#124](https://github.com/AruNi-01/atmos/pull/124)).
- **Companion Beta Alignment** — Bundled the latest Atmos CLI and local web runtime beta updates so desktop beta.11 stays aligned with `0.2.0-beta.8` across companion install/update paths.
- **Spec and Coverage Artifacts** — Checked in the APP-021 planning, review, and regression coverage artifacts that describe and verify the new Appshots release slice.

---

> **Beta release.** Beta.10 supersedes beta.9. It carries all beta.9 content plus the automation and agent-session updates below. Please report any issues you encounter so we can land them before the stable release.

## Changes Since 1.2.0-beta.9

- **Local Automations** — Added the first local automation workflow surface, including scheduled and manual runs, terminal-agent execution, local run artifacts, desktop/web notifications, management-center entry points, and GitHub-trigger foundations for future event-driven automations ([#122](https://github.com/AruNi-01/atmos/pull/122)).
- **GitHub Automation Reliability** — Hardened GitHub trigger setup, relay routing, delivery dedupe, access-token rotation behavior, and D1 schema handling so automation routes recover more predictably across relay and local runtime changes.
- **ACP Session Lifecycle Upgrade** — Upgraded ACP session handling with context-aware session lists, cwd/workspace filters, replayed chat history, session-root aggregation, and clearer unsupported-state handling ([#123](https://github.com/AruNi-01/atmos/pull/123)).
- **Desktop Runtime & App Stability** — Bundled system skills for the dev desktop runtime, avoided WebSocket bootstrap self-wait, refreshed remote-computer connection data, and fixed a Claude usage refresh edge case.
- **Bundled Beta Components** — Included the latest Atmos CLI and local web runtime beta updates so the desktop package stays aligned with the companion command-line and hosted/local web entry points.
- **Docs and Structure** — Refreshed user guides and clarified store/connection boundaries to keep the new automation, remote connectivity, and ACP flows easier to maintain.

---

> **Beta release.** Beta.9 supersedes beta.8. It carries all beta.8 content plus the improvements below. Please report any issues you encounter so we can land them before the stable release.

## Changes Since 1.2.0-beta.8

- **Unified diff code views** — Review and Changes now share the same code-review presentation, with aligned toolbar icons, separator layout, theme colors, and persisted diff viewer settings ([#120](https://github.com/AruNi-01/atmos/pull/120)).
- **Leaner app architecture** — Large-file refactors and layout cleanup improve maintainability; the PR modal action toolbar now peeks on hover instead of staying always visible ([#121](https://github.com/AruNi-01/atmos/pull/121)).
- **Welcome & terminal polish** — The welcome composer footer stays inside its card, and the terminal grid stylesheet import is corrected.
- **Stability & tooling** — WebSocket handling is more stable, desktop debug static builds are simpler, and documentation deploys to Cloudflare Pages with an updated landing docs link.
