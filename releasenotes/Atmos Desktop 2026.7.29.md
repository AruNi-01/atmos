Atmos Desktop 2026.7.29 is the production desktop release that graduates the 2026.7.27-beta.1 dogfood line. It ships the production Atmos shell (product identity `Atmos` / `com.atmos.desktop`) with bundled Server runtime, native AppShot dual-shift capture, binary-safe git diffs with image previews, packaging polish, and reliability fixes across skills, terminal, and refresh flows.

## New Features

- Shipped the production Atmos desktop package path with bundled Atmos Server runtime, shared on-disk contracts (`~/.atmos/appshots`, `~/.atmos/desktop`, tunnel gateway + entry token), and app identity `com.atmos.desktop`.
...
- Fixed macOS dogfood Gatekeeper “damaged” packages with ad-hoc codesign of sealed resources.
