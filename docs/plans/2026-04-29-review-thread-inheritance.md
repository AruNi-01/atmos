# Review Session Thread Inheritance 技术方案
...
1. **Threads 消失** — 切换到 V2 后看不到任何 V1 的评论线程
...
核心原因：`finalize_fix_run` 创建新修订版时只继承了 file state（`reviewed` 标记），但没有继承 review threads，也没有将 agent 的回复消息迁移到新版本。

---

## 设计目标

1. V2 的 Threads 应包含 V1 的用户评论和 Agent 回复（完整评论链）
2. 数据不冗余 — 消息不复制，通过 `parent_thread_guid` 关联
...
```
review_thread.parent_thread_guid  -- 指向父版本中的原始线程
```
...
```
V1 Thread A (parent_thread_guid = NULL)
  ├── message: user "为什么要引入这个包？"
  ├── message: user "只用来做测试吗？"
  │
  │  finalize_fix_run 迁移:
  │  - agent 回复消息 → 从 Thread A 移到 Thread A'
  │
  └── V2 Thread A' (parent_thread_guid = Thread A.guid)
       ├── (从 Thread A 迁移过来) message: agent "vitest 是为了支持..."
...
b. 识别有 parent_thread_guid 的线程
c. 递归解析父链 (resolve_ancestor_threads)，缓存所有祖先线程
d. 批量加载祖先线程的消息
e. 对有 parent_thread_guid 的线程：
   - 本线程自身消息 + 父链所有祖先线程的消息
   - 按 created_at 排序合并
f. 返回完整的 ReviewThreadDto
```

这样 V2 的线程 DTO 的 messages 数组会包含：
- 本线程上的 agent 回复（从 V1 迁移过来的）
- 父链上 V1 线程的用户原始评论（通过 parent_thread_guid 追溯）
...
│  4. 继承 review_thread:                                     │
│     V1 Thread A ──→ V2 Thread A'                            │
│       (parent_thread_guid = A.guid)                         │
│  5. 迁移 agent 消息:                                        │
│     message(fix_run=run1, thread=A) ──→ thread=A'           │
...
```
V2 Thread A' (parent_thread_guid = A.guid)
  │
...
`resolve_ancestor_threads` 递归解析父链：
- V3 Thread 的 parent 指向 V2 Thread
- V2 Thread 的 parent 指向 V1 Thread
- 查询 V3 Thread 时，消息列表 = V3自身 + V2父 + V1祖先
...
3. **UI 展示优化**: 在 ThreadCard 中展示 parent 关系（如 "继承自 V1 评论" 标签）