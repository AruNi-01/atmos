...
> **APP-062 (2026-08-15):** live I/O 不再走 per-browser `tmux -C` / `send-keys -H` / `%output`。唯一 live 路径是 master pane 上的一条 `pipe-pane -I -O`，由 API `PaneIoRegistry` fan-out 到 N 个 WebSocket。本 ADR 的 **persistence 模型不变**：master tmux session + named window、`capture-pane` snapshot 只做 remount hydration、JSON 控制 + binary output 的 `/ws/terminal/:id`。control-mode parser（`tmux/control.rs`）不再是 live transport。详见 [APP-062 TECH](../../specs/APP/APP-062_terminal-tmux-pipe-live-path/TECH.md)。
