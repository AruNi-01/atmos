...
impl WorkspaceService {
    ...
    pub async fn ensure_worktree_ready(&self, guid: String) -> Result<()> {
        ...
        if existing_branches.contains(&workspace.branch) {
            tracing::warn!(
                "[ensure_worktree_ready] Branch '{}' already exists, regenerating a new name",
                workspace.branch
            );
            let prefix = workspace_name_generator::extract_repo_prefix(&project.name);
            workspace_name =
            ...
            tracing::info!(
                "[ensure_worktree_ready] New name generated: {}",
                workspace_name
            );
        ...
        match self
            .git_engine
            .create_worktree(repo_path, &workspace_name, &base_branch)
        {
            Ok(created_path) => {
                tracing::info!(
