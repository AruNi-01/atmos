...
pub fn permission_doctor(manager: &DesktopUseManager) -> PermissionDoctor {
    ...
    notes.push(scrub_vendor(
        "Grant Accessibility and Screen Recording to Atmos Desktop Use (not any third-party driver name) for AppShot and desktop control. In Atmos Desktop, Settings → Desktop Use → Grant Accessibility opens System Settings and a drag panel for the host app when it is missing from the list.",
    ));
