...
`--permission-mode` is a **parent** flag. `grok agent --permission-mode default stdio` is rejected (`unexpected argument '--permission-mode'`). Composer-selected ids: `default` / `plan` / `auto` / `bypassPermissions`. Still omit `--always-approve` / `--yolo`. Mid-session: `/always-approve` + `/auto` slash via `session/prompt` (`AcpSessionControl::prompt_turn`); Plan via `session/set_mode`. Those slash RPCs must **not** emit Atmos `TurnEnd` / `TurnCompleted` (they are control prompts, not user turns — otherwise send-time `apply_pending_session_config` races and leaves Streaming stuck). Do not add Grok `session/new` `yoloMode` to the generic ACP mapper (spawn `--permission-mode` owns create-time YOLO).
...
| `_x.ai/ask_user_question` | **Inbound** agent → client request (not Atmos→Grok). Params `{ sessionId, toolCallId, questions[], mode }`. Each question has `question`, `options[{label,description}]`, optional `multiSelect` / `header`. Atmos maps to `PermissionRequested` + ApprovalCard `questions`; respond with serde internally tagged **`outcome`** (not `type`): `{ outcome: "accepted", answers: { <question text>: <label\|label[]> }, annotations: {} }` or `{ outcome: "skip_interview" }`. A bare `{ type: "accepted" }` fails live CLI with `missing field \`outcome\``. Companion `session/update` `tool_call` title `ask_user_question` is **hidden** (no tool card). Fixtures: `ask_user_question.json`, `ask_user_question_response.json`, `ask_user_question_skip.json`, `tool_call_ask_user.json` (live main `/ws` 2026-09-05). |
| `_x.ai/exit_plan_mode` | **Inbound** agent → client request. Params `{ sessionId, toolCallId?, planContent?, plan? }`. Atmos maps to `PermissionRequested` + ApprovalCard `plan` (`ExitPlanMode`). Respond with JSON-RPC **success** `{ outcome: "approved" \| "cancelled" \| "abandoned" }` (`approved` = build, `cancelled` = keep planning / revise, `abandoned` = leave plan mode). A Method not found / JSON-RPC error is treated as client disconnect (`Plan approval could not be completed…`). Companion `tool_call` title `exit_plan_mode` is **hidden**. Enter/exit Plan chrome mid-session still uses ACP `session/set_mode` (`plan`/`default`). |

**Native vs ACP:** Chat provider id `grok` is a **native** host (`GrokNativeProvider`), but the published CLI wire is still **ACP JSON-RPC over stdio** (`grok agent stdio` + `_x.ai/*`). Logs showing `agent_client_protocol` / `session/update` are expected. That is not the same spawn path as registry ACP ids (`grok-build` / `grok-acp` / Cursor), which also use `acp_client` but different argv.
