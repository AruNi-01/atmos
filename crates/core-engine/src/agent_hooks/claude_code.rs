...
fn build_cmd(port: u16, json_body: &str) -> String {
    ...
    format!(
        r#"[ "$ATMOS_MANAGED" = "1" ] && {hook_version} && curl -sf -X POST -H 'Content-Type: application/json' -H "X-Atmos-Context: $ATMOS_CONTEXT_ID" -H "X-Atmos-Pane: $ATMOS_PANE_ID" -H "X-Atmos-Terminal-Kind: $ATMOS_TERMINAL_KIND" -H "X-Atmos-Side-Chat-Id: $ATMOS_SIDE_CHAT_ID" -H "X-Atmos-Source-Pane: $ATMOS_SOURCE_PANE_ID" {hook_version_header} -d '{json_body}' '{url}' >/dev/null 2>&1 || true"#,
