...
3. **AST artifacts loaded/verified** — Read `.atmos/wiki/_ast/` artifacts generated by backend before writing docs.
4. **validate_catalog passes** — Run `~/.atmos/skills/.system/project-wiki/scripts/validate_catalog.py` or `.sh`.
5. **validate_frontmatter passes** — Run `~/.atmos/skills/.system/project-wiki/scripts/validate_frontmatter.py`.
6. **validate_content passes** — Run `~/.atmos/skills/.system/project-wiki/scripts/validate_content.py` or `.sh`.
7. **validate_todo passes** — Run `~/.atmos/skills/.system/project-wiki/scripts/validate_todo.py` or `.sh`.
8. All `_todo.md` items checked `[x]` — Do not mark complete until every item is checked.
...
### Step 2.5: Load AST Artifacts from Backend (Mandatory Unless Missing)

This skill assumes AST indexing is generated by backend before the local Agent starts. Load and verify these artifacts from `./.atmos/wiki/_ast/`:

- `index.json` — file-level index (which shard file to open per source file)
- `hierarchy.json` — directory tree index for progressive drill-down
- `files/*.json` — per-file AST shards (symbols + relations + parser warnings)
- `symbols.jsonl` — global symbol stream (fallback/debug)
- `relations.jsonl` — global relation stream (fallback/debug)
- `_status.json` — metadata (`generated_at`, `commit_hash`, language stats, parse warnings)

Validation rules:
1. If `_status.json` exists, compare its `commit_hash` to current `git rev-parse HEAD`. If stale, note drift and treat AST as partial guidance.
2. If AST artifacts are missing, continue in **degraded mode** (source+metadata only), and record this in `_todo.md`.
3. **Progressive disclosure required**: read `hierarchy.json`/`index.json` first, then open only relevant `files/*.json` shards for the current article; do NOT load all AST shards for large repos.
4. Never fabricate AST evidence. If relation data is missing, explicitly mark inference as "source-read inference".

Update `_todo.md`: check `[x] AST artifacts loaded/verified` (or `[x] AST unavailable, degraded mode acknowledged` if missing).
...
- **AST evidence bundle** (if available): key symbols, critical relations, and at least one candidate request/data-flow chain
...
2. **Spawn Subagents**: For each item, spawn a subagent with:
   ...
   - **AST evidence subset** from `.atmos/wiki/_ast/` scoped to this page's topic/sources
...
When AST artifacts are available, ground key claims (APIs, dependencies, call paths) in AST evidence first, then enrich with source and metadata context.
