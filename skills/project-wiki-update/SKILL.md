...
If AST artifacts are available, use `relations.jsonl` and per-file shard references as candidate evidence for indirect impact (e.g. import/use references, call sites, shared type hubs). These are syntactic references, not a resolved call graph — confirm transitive caller/callee or module-impact edges against source before treating them as definitive.
...
If `.atmos/wiki/_ast/relations.jsonl` is available, also build a tentative `symbol/module -> [wiki_page_paths]` mapping from syntactic references for candidate impact expansion. Verify candidates against source before committing to page updates.
...
- If AST is available, consider pages that may be transitively impacted through changed symbols/modules (not only direct file overlap). AST-derived references are syntactic — confirm against source before treating as definitive impact.
