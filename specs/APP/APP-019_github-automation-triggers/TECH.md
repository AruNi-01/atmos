...
- When APP-019 is implemented, update `packages/relay/AGENTS.md` so future agents understand that provider event ingress is allowed routing/auth logic, not Atmos business execution.
...
- **Token rotation**: APP-020 owns stable tenant identity and Access Token rotation. APP-019 consumes that tenant model; it must not copy GitHub rows across tenants.
- **Lost token**: If the user cannot present the old token, APP-019 does not provide ownership transfer for GitHub installations or routes. The user must create a new token identity and set up Computer/GitHub triggers again.
...
APP-017 stores automation schedule and target metadata. APP-019 needs a trigger union that can represent manual/scheduled/github. If the current schema only stores schedule fields, add columns:
...
- Depends on APP-020 stable Relay tenant identity for Access Token rotation semantics.
