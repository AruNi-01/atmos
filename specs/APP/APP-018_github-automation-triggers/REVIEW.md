# REVIEW · APP-018: GitHub Automation Triggers
...
GitHub route creation, update, and deletion are control-plane operations. They must be authorized by the user's Relay Access Token and scoped to the stable APP-019 tenant. A route id supplied by a client must never let one tenant update or disable another tenant's route.
...
- Treat APP-019 direct token replacement as an identity switch: do not transfer routes, and require setup under the new token.
