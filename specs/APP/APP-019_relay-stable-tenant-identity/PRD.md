# PRD · APP-019: Relay Stable Tenant Identity
...
- **Why now**: APP-018 GitHub Automation Triggers adds long-lived GitHub App installations and event routes. Token replacement must not silently detach those routes when the user still has the old token locally.
...
- **M4 · Ownership preservation**: Rotation preserves tenant-owned rows, including registered Computers, server secrets, client-visible Computer metadata, and APP-018 GitHub installations/routes.
...
- **GitHub App reinstall logic**: APP-018 owns GitHub setup UX; APP-019 only preserves tenant ownership through rotation.
...
- **Leading**: APP-018 GitHub routes remain associated with the same tenant after rotation.
...
- **Phase 4 · APP-018 integration**: Ensure GitHub setup/routes use stable tenant ids before production rollout.
