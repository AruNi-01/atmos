# TEST · APP-019: Relay Stable Tenant Identity

> Test Plan · how we verify Relay Access Token rotation preserves tenant-owned resources safely. References PRD APP-019 and TECH APP-019.
...
- **When**: the APP-019 migration runs.
...
- **Level**: Relay integration
- **Given**: a tenant with Computers and APP-018 GitHub installation/route rows.
...
- **Level**: Relay integration
- **Given**: a tenant has unused register tokens, active client sessions, APP-018 setup sessions, registered Computers, and GitHub routes.
...
- [ ] APP-018 GitHub routes keep their tenant after rotation.
...
- [ ] APP-018 TECH references stable tenant identity instead of cross-token row copying.

## Manual verification steps

1. Copy a staging D1 database with at least one registered Computer.
2. Apply APP-019 migrations to the copy.
...
6. If APP-018 staging tables exist, confirm GitHub route rows still have the same stable `tenant_id`.
