...
- **M6**: Agent Chat history is sourced from ACP `session/list` for the selected agent. Atmos must not use its own persisted Agent Chat session table as a durable history source, and it must fetch session history one ACP cursor page at a time instead of recursively loading every page. When Agent Chat is opened inside a Project or Workspace with a resolved path, history must pass that path as the ACP `cwd` filter. Global agent session management defaults to an unfiltered All view, with an explicit Project/Workspace filter that passes the selected context path as `cwd`.
