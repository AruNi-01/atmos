...
- **M5 — Typed with APP-048**: `request(action: WsAction, data?: unknown): Promise<T>` with caller-supplied `T`. Frames from api-types. **Hard gate**: APP-048 Phase 1 (frames + actions) before 049 Phase 1. **Follow-on**: [APP-064](../APP-064_api-contract-hardening/PRD.md) maps `action → input → output` and tightens these generics without turning the kernel into a domain SDK.
