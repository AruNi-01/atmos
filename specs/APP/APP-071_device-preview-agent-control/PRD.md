...
- **M13**: Skill discovery when the user did not provide a device id (no `--udid`, no Device Preview chip, no “the Pixel in workspace X”):
  ...
  2. Show each row as **name · platform · project or workspace** (mark “this workspace” when it matches Chat/CLI context).
...
- **M14**: The Simulator tab (Device Preview) has an Atmos-owned **Agent** control (overlay on `SimulatorPanel`, **not** inside vendored serve-sim / serve-emu chrome). When the preview is live, click copies the Agent prompt for **this tab’s claim** (udid, name, platform, and the non-empty project **or** workspace). Clipboard uses the existing AI-context protocol so paste into an Atmos composer becomes a chip. Button shows inline **Copied** (no success toast). English sentence case. `en.json` + `zh.json`. Prompt never includes helper `url` / `port` / tokens. Skill instruction uses the synced path `~/.atmos/skills/.system/atmos-device-preview/SKILL.md`, not a bare skill name.
