...
Issue #26 需要解决的不是“桌面端能不能启动 web 服务”这一单点问题，因为当前 Tauri + 统一本机 runtime 已经具备本地服务启动与前端静态资源服务能力；真正需要定义的是：如何把 Atmos 的本地运行实例以尽量少配置、尽量自动化、同时安全可控的方式开放给隧道连接器。
...
但当前安全模型不适合作为隧道连接器入口。`apps/api/src/config/mod.rs` 默认绑定 `0.0.0.0`，而 `apps/api/src/middleware/mod.rs` 对 loopback/LAN 默认信任，仅对更远端来源要求 token。这对于本地开发很方便，但在接入 Tailscale、Cloudflare Tunnel、ngrok 之后，请求很可能在进入应用时表现为本机或局域网来源，导致“来源 IP 是否可信”失去区分本地访问与经隧道转发访问的能力。
## 设计目标
新的技术方案应满足以下目标：
...
2. 隧道连接器核心层：统一的 TunnelGateway、TunnelSession、provider 抽象与代理逻辑。
3. 宿主控制层：桌面端等 host/control surface，负责在本机启动和管理隧道连接器。
其中第二层应被设计成一个独立子系统，而不是直接散落在 `apps/api` 或 `apps/desktop` 内部。
### 二、单独抽取 `crates/tunnel-connector`
建议新增独立 crate：`crates/tunnel-connector`。
这个 crate 承载真正可复用的隧道连接器能力，而不是桌面端专属逻辑。它的职责包括：
...
* 托盘、窗口、生命周期管理
* 系统层凭证录入交互
这样能把“隧道连接器子系统”和“桌面宿主适配层”彻底分开。
### 三、桌面端只保留 Tunnel Connector Manager
桌面端仍然需要一个 `TunnelConnectorManager`，但它不再承载全部隧道连接器逻辑，只负责宿主层编排。建议保留在 `apps/desktop/src-tauri/src/tunnel_connector/`。
它的职责包括：
* 检测本机可用 provider
* 启动和关闭 `crates/tunnel-connector` 中的 gateway/runtime
* 管理宿主态状态和生命周期
...
* 调整当前 local/LAN trust 逻辑，使其不再承担隧道连接器安全边界
换句话说：`apps/api` 是被代理目标，不是对外发布系统。
## Tunnel Gateway 设计
### 一、Gateway 的定位
Tunnel Gateway 是隧道连接器的统一入口，不直接等同于业务 API server。它负责：
* 接受浏览器访问
* 代理 HTTP 请求到内部应用层
* 代理 WebSocket 握手与消息流
* 处理分享会话校验
* 返回分享错误页、过期页、无权限页等入口级响应
它是“隧道连接器边界”，而不是“业务应用本体”。
### 二、为什么要有 Gateway
...
## TunnelSession 设计
隧道连接器不能仅依赖 provider 自带认证，必须有应用内统一会话模型。建议定义 Tunnel`Session`，至少包含：
* `session_id`
* `provider`
* `mode`（private/public）
...
## Provider 抽象
### 一、统一 provider trait
在 `crates/tunnel-connector` 中定义统一 provider 接口，至少包括：
...
### 二、桌面宿主层
建议新增：
* `apps/desktop/src-tauri/src/tunnel_connector/mod.rs`
* `apps/desktop/src-tauri/src/tunnel_connector/manager.rs`
* `apps/desktop/src-tauri/src/tunnel_connector/commands.rs`
桌面层通过 `AppState` 持有 manager/runtime 引用，并通过 Tauri commands 暴露启停、状态查询、复制链接、诊断等操作。
### 三、API 配合改动
`apps/api` 中只做配合性调整，不承担隧道连接器编排：
* 收紧默认监听地址
* 收敛当前 LAN 信任逻辑
...
### 2. Tunnel Connector 模式不再依赖来源 IP 判定可信
当前 `is_local_or_lan()` 只能作为本地开发便利逻辑保留，不能再承担隧道连接器安全边界。隧道连接器必须切换到 Tunnel`Session` 模型。
