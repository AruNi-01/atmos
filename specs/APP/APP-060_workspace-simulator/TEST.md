# TEST · APP-060: Workspace Simulator
...
|----|-------|--------|----------------|---------|--------|
| S1 | e2e | `e2e/tests/specs/APP-060_workspace-simulator.e2e.ts` | stub bridge, probe ok | one `simulator_attach` for two surfaces | pending |
...
| S8 | e2e | APP-060 e2e, setup-card group | stub per probe code | title + reason + facts + button per code; no shell string as primary CTA | pending |
| S9 | e2e | APP-060 e2e, setup-card group | stub without `__ATMOS_DESKTOP__` | "Requires Atmos Desktop"; zero invokes | pending |
...
| S21 | e2e | APP-060 e2e, fallback group | stub driving fallback | last frame or skeleton visible at all times; no blank canvas | pending |
