...
| `apps/desktop`            | 启动时 **`runtime-manager::supervisor::ensure_running`**（与 CLI / `npx @atmos-land/local-web-runtime` 共用同一 API 进程）；**不再**使用 Tauri sidecar + 每实例 `ATMOS_LOCAL_TOKEN`。退出 Desktop **不**终止共享 API。 |
