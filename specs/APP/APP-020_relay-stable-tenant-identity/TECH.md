# TECH · APP-020: Relay Stable Tenant Identity

> Technical Design · HOW. Implements PRD APP-020: Relay Stable Tenant Identity.
...
flowchart LR
  ...
  D1 --> GitHub["APP-019 github_* tables"]
...
- APP-019 implementation: GitHub tables reference stable `tenant_id`.
...
- APP-019 `github_setup_sessions.tenant_id`
- APP-019 `github_app_installations.tenant_id`
- APP-019 `github_event_routes.tenant_id`
- APP-019 `github_webhook_deliveries.tenant_id`
...
Repeat the same pattern for `register_tokens` and `client_sessions`. APP-019 migrations should be written against the new stable-tenant schema; if APP-019 tables already exist in an environment, add them to the same rebuild pattern.
...
- Delete or invalidate short-lived tenant-scoped rows:
  - `register_tokens`
  - `client_sessions`
  - APP-019 `github_setup_sessions`
- Keep durable rows:
  - `computers`
  - APP-019 `github_app_installations`
  - APP-019 `github_event_routes`
  - APP-019 `github_webhook_deliveries`
...
If APP-019 tables are not yet deployed, `github_setup_sessions` cleanup should be feature-detected or added with APP-019's migration.
...
### APP-019 GitHub route integration

APP-019 GitHub tables must use stable `tenant_id` values. Rotation should not rewrite GitHub route rows; the tenant id remains unchanged.
...
7. Update APP-019 implementation to create GitHub rows with stable tenant ids.
...
- **Tradeoff**: Stable tenant id adds one migration now but removes route/table rewrite complexity from APP-019 and future providers.
