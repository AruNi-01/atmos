...
Options:
  -p, --port <port>      Port to listen on (default: 3300)
      --host <addr>      Loopback bind only (default: 127.0.0.1). Non-loopback
                         values are ignored.
...
async function main() {
  ...
  const requestedHost = values.host ?? DEFAULT_HOST;
  // Atmos: always bind loopback. Ignore LAN/wildcard --host.
  const host = isLoopbackHost(requestedHost) ? requestedHost : DEFAULT_HOST;
