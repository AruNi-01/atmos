...
/** Atmos fork: never bind a public interface. */
function loopbackHost(host: string | undefined): string {
  if (host === "localhost" || host === "::1" || host === "127.0.0.1") return "127.0.0.1";
  return "127.0.0.1";
}
...
program
  ...
  .option(
    "--host <addr>",
    "Ignored unless loopback. Atmos always binds 127.0.0.1.",
    "127.0.0.1",
  )
...
Examples:
  ...
  .action(async (devices: string[], opts) => {
    ...
    if (opts.detach) {
      ...
      await serve(startPort ?? 3200, devices, startPort !== undefined, loopbackHost(opts.host), {
        stream,
