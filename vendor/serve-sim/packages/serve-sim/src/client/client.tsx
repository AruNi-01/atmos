        ...
        {selectedDevice ? (
          ...
          <div className="flex flex-col items-center gap-3 text-center">
            ...
            <p className="text-white/55 text-[14px] max-w-120">
              Create an iPhone simulator in Xcode, then try again.
            </p>
          </div>
        )}
      </div>
...
function AppWithConfig({
  ...
  // Tools is a first-class pane (same as Devices). Remember the last open
  // state; first visit stays closed.
  const [panelOpen, setPanelOpenState] = useState(() => {
    if (typeof window === "undefined") return false;
    const stored = window.localStorage.getItem("serve-sim:tools-panel-open");
  ...
  const setPanelOpen = (next: boolean | ((open: boolean) => boolean)) => {
    setPanelOpenState((open) => {
      ...
      try {
        window.localStorage.setItem("serve-sim:tools-panel-open", resolved ? "1" : "0");
      } catch {}
      return resolved;
    });
  };
