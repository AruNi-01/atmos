...
/// Atmos-only additive API: graph + model + monthly reports from one local scan/parse.
/// Keep upstream public report entry points unchanged so vendored upgrades rebase cleanly.
#[derive(Debug, Clone)]
...
/// Build all three token-usage report shapes from a single local session pass.
pub async fn generate_usage_reports(options: ReportOptions) -> Result<UsageReports, String> {
    ...
    let entries = aggregate_model_usage_entries(filtered.clone(), &options.group_by);
    let (total_input, total_output, total_cache_read, total_cache_write) =
        model_report_token_totals(&entries);
    let total_messages: i32 = entries.iter().map(|e| e.message_count).sum();
    let total_cost: f64 = entries.iter().map(|e| e.cost).sum::<f64>() + 0.0;
    let model_report = ModelReport {
        ...
        total_messages,
        total_cost,
        processing_time_ms: model_start.elapsed().as_millis() as u32,
    };
    ...
    let mut monthly_entries: Vec<MonthlyUsage> = month_map
        .into_iter()
        .map(|(month, agg)| MonthlyUsage {
            month,
            models: agg.models.into_iter().collect(),
        ...
        .collect();
    monthly_entries.sort_by(|a, b| a.month.cmp(&b.month));
    let monthly_total_cost: f64 = monthly_entries.iter().map(|e| e.cost).sum::<f64>() + 0.0;
    let monthly_report = MonthlyReport {
        entries: monthly_entries,
        total_cost: monthly_total_cost,
        processing_time_ms: monthly_start.elapsed().as_millis() as u32,
    };

    let bucket_timezone =
        bucket_tz::BucketTimezone::from_scanner_settings(&options.scanner_settings);
    let mut graph = build_graph_from_messages(
        filtered,
        pricing.as_deref(),
        GraphPricingRequirement::Lenient,
        start,
