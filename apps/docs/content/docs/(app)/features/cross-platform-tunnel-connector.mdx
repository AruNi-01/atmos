...
Tunnel Connector is desktop-only. It exposes the current computer's local Atmos runtime through a supported tunnel provider. It is different from [Atmos Computer](/features/atmos-computer), which registers a machine with Relay so other clients can select it as their active computer.
