              ...
              {(slots.languageSelect || slots.themeSwitch) && (
                <div className="flex items-center ms-auto">
                  ...
                  {slots.themeSwitch && (
                    <slots.themeSwitch
                      className={cn(
                        'px-1 py-0 border-y-0 border-e-0 rounded-none *:rounded-md',
                        slots.languageSelect && 'border-s border-fd-border',
                      )}
                    />
                  )}
                </div>
          ...
          <div className="flex text-fd-muted-foreground items-center gap-1.5">
            ...
            {(slots.languageSelect || slots.themeSwitch) && (
              <div className="flex items-center">
                ...
                {slots.themeSwitch && (
                  <slots.themeSwitch
                    className={cn('p-0', slots.languageSelect && 'border-s border-fd-border')}
                  />
                )}
              </div>
            )}
...
function LayoutTabIcon({
  ...
  return (
    <div
      className={cn(
        'flex shrink-0 items-center justify-center text-fd-foreground',
        size === 'lg'
          ? 'size-9 rounded-md border bg-fd-secondary/60 [&_svg]:size-5'
          : 'size-5 rounded border bg-fd-secondary/60 [&_svg]:size-3.5',
      )}
    >
      {children}
    </div>
...
function SidebarTabsDropdown({
  ...
  const item = selected ? (
    <>
      {selected.icon ? <LayoutTabIcon size="sm">{selected.icon}</LayoutTabIcon> : null}
      <p className="min-w-0 flex-1 text-sm font-medium leading-5">{selected.title}</p>
    </>
  ) : (
    placeholder
  );
      ...
      <PopoverContent className="flex flex-col gap-1 w-(--radix-popover-trigger-width) p-1 fd-scroll-container">
        {tabs.map((item) => {
          ...
          return (
            ...
            >
              {item.icon ? <LayoutTabIcon>{item.icon}</LayoutTabIcon> : null}
              <div className="min-w-0 flex-1">
                <p className="text-sm font-medium leading-none">{item.title}</p>
                {item.description ? (
                  <p className="mt-1 line-clamp-1 text-xs text-fd-muted-foreground">
                    {item.description}
                  </p>
                ) : null}
              </div>
