...
export const changelogData: ChangelogItem[] = [
  {
    id: "desktop-2026.8.23",
    title: {
      zh: "Prototype Design · 独立中心空间 · 全屏舞台",
      en: "Prototype Design, Center Spaces & Fullscreen Stage",
    },
    description: {
      zh: "Atmos Desktop 2026.8.23 新增 Prototype Design、可命名的独立中心空间，以及全屏中心舞台。工作区 setup 收进顶栏 jobs chip，Settings 重新分组，Token 用量可按 Computer 查看，系统浏览器登录后可一键回到 Atmos。",
      en: "Atmos Desktop 2026.8.23 adds Prototype Design, named independent center spaces, and a fullscreen center stage. Workspace setup moves into the header jobs chip, Settings is regrouped, Token Usage can be scoped by Computer, and signing in through the system browser now returns you to Atmos.",
    },
    date: "2026-08-23",
    version: "2026.8.23",
    releaseUrl:
      "https://github.com/AruNi-01/atmos/releases/tag/desktop-electron-2026.8.23",
    tags: [
      { zh: "Prototype Design", en: "Prototype Design" },
      { zh: "中心空间", en: "Center Spaces" },
      { zh: "Workspace jobs", en: "Workspace jobs" },
      { zh: "Token 用量", en: "Token Usage" },
    ],
    content: {
      zh: {
        features: [
          "**Prototype Design** — 从 Launchpad、左侧栏或工作区标签打开 Agent 优先的线框画板。从可搜索目录放置组件与区块，本地 Agent 可编辑当前画板，也可邀请他人同画。协作邀请链接可打开全屏访客画板，无需 computer 引导。",
          "**中心空间** — 一个工作区可保留多个命名 mosaic。额外空间从空白开始、共用同一仓库，切换时保持预热。需要关注的工作区会点亮空间切换器；agent hook 打开拥有该会话的空间。",
          "**中心全屏** — 把窗格铺满页脚上方的舞台。应用命名布局前会先确认，避免覆盖当前分屏。",
          "**多窗格中心舞台** — Changes、Review、Files、Run、GitHub 作为中心工具标签。舞台可拆成已保存布局、启动空白窗格，并可通过标题行拖动终端窗格。",
          "**Workspace jobs** — setup 进度改在顶栏 jobs chip，不再占满 overlay。已就绪的工作区仍留在 chip 上，不用离开当前上下文就能打开。",
          "**Settings 分组** — You、Workspace、Agents、System，页内标签覆盖 General、Editor、Workspace、Remote Access、Apps。再次打开 Settings 会回到上次所在分区。",
          "**按 Computer 看 Token 用量** — 选择一台 Computer、经 Relay 查看另一台机器，或看 All computers 合计且不重复计算同一 Cursor 账号。未登录或只有一台 Computer 时，选择槽显示 Sign in / Add Computer。",
          "**Launchpad 布局** — 长按卡片或列表项即可重排，刷新后仍记住位置。",
          "**Back to Atmos** — 系统浏览器 OAuth 完成后，Back to Atmos 会聚焦正在运行的桌面窗口。",
        ],
        fixes: [
          "**Shell 崩溃** — 打开应用不再因侧栏 workspace job id 陷入 React 更新深度循环。",
          "**窗口闪白** — 原生窗口加载时不再闪成浅色。",
          "**Terminal** — 缩放不再闪烁或残留换行；重连 hydration 保持有序；不会因命令名误开 TUI 鼠标；选择工具栏保持单行；grok 的 tmux 检测留在子 PTY。折叠的 AI 输入贴在窗格边缘上方；侧边聊天保留 agent 图标、可点的缩放边与更高 overlay 对比。进程退出后实时标题去掉残留 agent 品牌。Pierre diffs 在绘制前预热并使用 Atmos 主题。",
          "**Mosaic** — 额外空间保留各自的中心上下文、标签 chrome、Run 资源与 tmux 窗格；关闭窗格不再跳回 Overview；dock 忽略来源窗格；文件编辑器仍托管在标签条外；新标签落在条尾。URL 中显式的 Changes 标签不再被持久化的 Files 标签覆盖。",
          "**Plus 菜单** — 悬停仍会打开；切换 Tabs / Layout 需要点击。Layouts 在前，fullscreen 在后。",
          "**Search** — 全局搜索保持 typeahead 焦点，并跟随命令列表中的指针悬停。",
          "**Account** — Account 资料页显示 GitHub 与 Google 头像。",
          "**复制与预览** — 访客 webview 在桌面锁定下可再次使用剪贴板与本地应用预览。",
          "**滚动** — 恢复 macOS overlay bounce；overlay 菜单把滚轮限制在内部；GitHub 抽屉留在中心舞台卡片内。",
          "**Quota 与 Linear** — Cursor 实时额度优先走 usage-summary；过期 Linear token 会刷新，而不再强制重连。",
          "**Folder Kanban 悬停** — 离开悬停列后不再保持淡出。",
          "**Disk Analyzer 扫描** — 扫描进行中，悬停或聚焦控件会显示 Cancel，而不再单独显示 scanning 标签。",
          "**通知** — 系统通知使用当前品牌图标。",
        ],
        improvements: [
          "**Prototype Design chrome** — 画板快捷键、库与主题留在画布上；agent 工具覆盖 layout 与 lint；agent island 与 Canvas 一致。交接会映射到目标项目已有的 UI 库。",
          "**Mosaic chrome** — 从 header 拖动窗格、分屏带动画、空白窗格启动器按 leaf 尺寸、已保存布局含 plus-menu 快照，空间切换器会把点中的 fan 卡片展开到中心。",
          "**应用图标** — Saturn 标志用于产品 UI、Dock 与 Desktop Use 宿主助手，DMG 图标保持连续 squircle 边。",
          "**Token 用量** — 独立加载页、Hub OAuth 后可再打开分享、公开 GitHub 卡片无需 computer、公开分享/排行榜保持最新，以及解释为何需要钥匙串的启用提示。",
          "**Welcome 标题** — 落地提示使用打字机动效与流星背景。",
          "**DMG 卷标** — Finder 卷标图标内缩，底板不再铺满桌面井。",
          "**Canvas** — 出现 tldraw 许可 overlay 时隐藏画板。",
          "**错误页** — 404 与服务器错误页增加 Home 操作。",
          "**Settings** — Browser Use、Desktop Use 与 Atmos Computer 共用实验性烧瓶提示；账户安全保留已关联账号与受信任设备，而不再列出 Hub 会话。",
          "**Chrome** — 交互控件的悬停填充不再带颜色过渡。",
        ],
        others: [
          "完整对比：https://github.com/AruNi-01/atmos/compare/desktop-electron-2026.8.16...desktop-electron-2026.8.23",
        ],
      },
      en: {
        features: [
          "**Prototype Design** — Open an Agent-first wireframe board from Launchpad, the left sidebar, or a workspace-scoped tab. Place components and blocks from a searchable catalog, let a local Agent edit the open board, and optionally invite someone else onto the same canvas. Collab invite links can open a fullscreen guest board without computer onboarding.",
          ...
          "**Workspace jobs** — Setup progress lives in the header jobs chip instead of a full overlay. Ready workspaces stay on the chip so you can open them without leaving the current context.",
          "**Settings groups** — You, Workspace, Agents, and System, with in-page tabs for General, Editor, Workspace, Remote Access, and Apps. Reopening Settings restores the last section.",
          "**Token Usage by Computer** — Pick a Computer, view another machine over Relay, or see an All computers total without double-counting the same Cursor account. When you are signed out or have only one Computer, Sign in / Add Computer is in the select slot.",
        ...
        fixes: [
          "**Shell crash** — Opening the app no longer hits a React update-depth loop from sidebar workspace job ids.",
        ...
        improvements: [
          ...
          "**App icon** — The Saturn mark is used across product UI, Dock, and the Desktop Use host helper, and the DMG icon keeps a continuous squircle rim.",
