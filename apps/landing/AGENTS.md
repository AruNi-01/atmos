...
`/tok` and `/tok/*` are **not** marketing routes. A Pages Function (`functions/_middleware.ts`) reverse-proxies them from `https://app.atmos.land` so the address bar stays `https://atmos.land/tok/@handle` (and `/tok/leaderboard`). Keep `/_next` URLs same-origin: the Function serves landing's own hashed assets when present and otherwise proxies the web app. Do not rewrite those URLs to `app.atmos.land` (Turbopack keys chunks by the exact script `src`). Do not add a Next.js `/tok` page here, and do not 302 those paths to `app.atmos.land`.
