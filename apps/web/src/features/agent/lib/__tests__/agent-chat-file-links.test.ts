...
describe("agent chat file-link wiring", () => {
  it("opens workspace files from assistant markdown like tool-result chips", () => {
    ...
    expect(source).toContain("ConversationHttpUrl");
    expect(source).toContain("withLinkifiedMarkdownComponents");
  ...
  it("expands read tool file previews for absolute paths outside the workspace", () => {
    ...
    expect(preview).toContain("max-w-52");
    expect(preview).toContain("rounded-xl");
    expect(preview).toContain("cursor-zoom-in");
    expect(preview).toContain("leading-none");
    expect(preview).toContain("className=\"block h-auto w-full\"");
    expect(block).toContain('body={imagePreview ? "plain"');
    expect(block).toContain("isImageToolPath(path)");
    expect(preview).not.toContain("max-h-32");
    expect(preview).not.toContain("max-w-56");
    expect(preview).not.toContain("max-h-96");
    expect(block).not.toContain('"hug"');
