...
/**
 * When a tool result has a path but no embedded content (common for writes
 * that only report diff_stats), fetch the file via /api/system/file — including
 * absolute paths outside the current workspace.
 */
    ...
    void (async () => {
      try {
        ...
        const clipped = text.length > PREVIEW_MAX_CHARS
          ...
          : text;
        const language = languageFromPath(absolute);
        if (language === "markdown" || /\.md$/i.test(absolute)) {
          setState({ kind: "markdown", text: clipped });
          return;
        }
        setState({ kind: "text", text: clipped, language });
      } catch {
        if (!cancelled) setState({ kind: "missing" });
      }
    })();
