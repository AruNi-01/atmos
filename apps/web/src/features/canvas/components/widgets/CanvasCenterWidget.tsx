...
function clientPointToLocalElementPoint(
...
function CanvasCenterTabContent({
  ...
  switch (tab.kind) {
    case "overview":
      return <CanvasCenterOverviewTab context={context} />;
    case "file":
...
function CanvasCenterOverviewTab({ context }: { context: CanvasContextRef }) {
  ...
  const { project, workspace } = React.useMemo(() => {
    ...
    const projectByWorkspace = context.workspaceId
      ? projects.find((item) => item.workspaces.some((workspaceItem) => workspaceItem.id === context.workspaceId))
      : undefined;
