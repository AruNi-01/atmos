...
export const PROVIDER_INSTALL: Record<string, ProviderInstallInfo | null> = {
  tailscale: {
    commands: {
...
export function ProviderActionTerminalPopover({
  provider,
  action,
  triggerLabel,
  triggerIcon,
...
export function ProviderActions({ p, onRedetect }: { p: ProviderDiagnostics; onRedetect: () => void }) {
  ...
  return (
    <div className="mt-2 space-y-1.5">
      ...
      {fallbackWarnings.map((warning, i) => {
        const parts = warning.split(/(`[^`]+`|https?:\/\/\S+)/g);
        return (
...
export function StartTunnelPopover({
  provider,
  isStarting,
  isStopping,
  onStart,
...
export function ViewTunnelPopover({
  status,
  onRenew,
}: {
  status: RemoteAccessStatus;
...
export function supportsTokenConfig(provider: ProviderKind): boolean {
