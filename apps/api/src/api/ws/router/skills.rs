...
impl WsMessageService {
    ...
    pub(super) async fn handle_skills_list(&self, req: SkillsListRequest) -> Result<Value> {
        use core_service::service::skill::{ScanMode, SkillScanner};
        ...
        async fn run_scan(project_paths: Vec<(String, String, String)>) -> Result<Vec<SkillInfo>> {
        ...
        fn strip_for_cache(skills: &[SkillInfo]) -> Vec<SkillInfo> {
            skills
                .iter()
                .map(|s| {
                    let mut s = s.clone();
        ...
        if let Some(cache) = cache.as_ref() {
            match cache.get::<Vec<SkillInfo>>(FEATURE, CACHE_KEY) {
                Ok(Some(entry)) => {
                    if entry.age >= TTL {
    ...
    pub(super) async fn handle_skills_get(&self, req: SkillsGetRequest) -> Result<Value> {
        use core_service::service::skill::SkillScanner;
