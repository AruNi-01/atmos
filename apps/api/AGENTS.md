...
- `POST /api/pt-design/agent/invoke` is non-browser Agent/CLI ingress (same category as `/api/canvas/agent/invoke`). The Server does not run PT Design tools; it forwards to the open board over `/ws` (`pt_design_agent_dispatch`). Users do not configure MCP or put `pt-design-mcp` on PATH.
