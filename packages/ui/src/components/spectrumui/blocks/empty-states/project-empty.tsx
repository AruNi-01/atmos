/**
 * Spectrum UI — ProjectEmpty
 *
 * First-project empty block with Templates, Checklist, and Minimal variants.
 * Installed from `@spectrumui/project-empty` and adapted for Atmos (semantic
 * tokens, reusable copy/actions, optional docs action).
 *
 * Source: https://ui.spectrumhq.in/r/project-empty.json
 * Dependencies: motion, @workspace/ui empty-state kit
 *
 * @example
 * <ProjectEmpty variant="Minimal" title="No agents yet" createLabel="Install hooks" docsLabel="New chat" />
 */
...
export interface ProjectEmptyProps {
  ...
  steps?: SetupStep[];
  createLabel?: string;
  /** Pass `null` to hide the secondary docs action. */
  docsLabel?: string | null;
  /** Replaces the default quiet docs button when set. */
  docsAction?: React.ReactNode;
  onCreate?: (templateId: string | null) => void;
  onDocs?: () => void;
  /** Replaces the default primary create button when set. */
...
export function ProjectEmpty({
  ...
  const showDocs = Boolean(docsLabel);
  ...
  const emptyState = (
      <EmptyState
        className={variant === 'Minimal' ? className : undefined}
        icon={icon}
        backdrop="stack"
        title={title}
        ...
        actions={
          <>
            {createAction ?? (
              ...
              >
                {variant !== 'Minimal' && chosen
                  ? `Create ${chosen.name.toLowerCase()} project`
                  : createLabel}
              </EmptyAction>
            )}
            {docsAction ??
              (showDocs ? (
                <EmptyAction emphasis="quiet" trailing={<IconArrowRight />} onClick={onDocs}>
                  {docsLabel}
                </EmptyAction>
              ) : null)}
          </>
              ...
              return (
                ...
                >
                  <span className="flex items-center justify-between">
                    <span className="text-foreground [&_svg]:size-[18px]">{template.icon}</span>
                    <AnimatePresence initial={false}>
                      {active && (
                        <motion.span
                          ...
                          transition={SPRING_SNAPPY}
                          className="text-foreground [&_svg]:size-4"
                        >
                          <IconTickSquare />
                        </motion.span>
                      )}
                  ...
                  <span className="mt-2.5 block text-[14px] font-medium text-foreground">
                    {template.name}
                  </span>
        ...
        {variant === 'Checklist' && (
          <div className="w-full max-w-[420px] text-left">
            <div className="mb-3 flex items-baseline justify-between">
              <span className="text-[13px] font-medium text-foreground">Setup</span>
                ...
                return (
                  <li key={step.id}>
                    <button
                      ...
                      className="flex w-full cursor-pointer items-start gap-2.5 rounded-xl px-2 py-2 text-left transition-colors duration-150 hover:bg-accent focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring"
                    >
                      <motion.span
  ...
  if (variant === 'Minimal') return emptyState;
