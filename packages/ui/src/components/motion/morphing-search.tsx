...
export type MorphingSearchItem = {
	...
	keywords?: string[];
	icon?: LucideIcon;
	/** Prefer over `icon` when a non-Lucide node (e.g. file-type img) is needed. */
	leading?: ReactNode;
	onSelect?: () => void;
};
...
export interface MorphingSearchProps {
	...
	open?: boolean;
	defaultOpen?: boolean;
	/** Seed the input on mount (e.g. restored session query). */
	defaultQuery?: string;
	/**
	 * When true (default), selecting a result closes the overlay.
	 * Pair with `clearOnSelect` to keep the query after navigation.
	 */
	closeOnSelect?: boolean;
	/**
	 * When true (default), selecting a result also clears the query.
	 * Set false with `closeOnSelect` so the overlay dismisses but query
	 * remains for the next open (e.g. Files empty-state keep-alive).
	 */
...
export function MorphingSearch({
	...
	// Files/Changes panels stay mounted via retainSurface + inert. The search
	// overlay is portaled to document.body, so close it when the host is inert
	// or we leave a floating panel over the next surface.
	useEffect(() => {
		if (!open) return;
		const anchor = anchorRef.current;
		if (!anchor) return;

		const dismissIfHostInert = () => {
			if (anchor.closest("[inert]")) {
				closeSearch({ clear: false });
			}
		};

		dismissIfHostInert();
		const observer = new MutationObserver(dismissIfHostInert);
	...
	useEffect(() => {
		if (open) {
			// Only sync an empty query on closed → open. Mounting already-open
			// (restored session) must keep `defaultQuery` / parent state.
	...
	const selectItem = useCallback(
		(item: MorphingSearchItem) => {
			item.onSelect?.();
			onSelect?.(item);
			if (closeOnSelect) closeSearch({ clear: clearOnSelect });
		},
		[clearOnSelect, closeOnSelect, closeSearch, onSelect],
	);

	// Prefer pointerdown over click: layout spring / highlight morph can move
	// the row between mousedown and mouseup so the click lands on the backdrop
	// (close without select) — Files empty-state then stays on the landing.
	const handleResultPointerDown = useCallback(
		(event: ReactPointerEvent<HTMLButtonElement>, item: MorphingSearchItem) => {
											...
											filteredItems.map((item, index) => {
												...
												return (
													<button
														...
														onPointerDown={(event) =>
															handleResultPointerDown(event, item)
														}
			...
			>
				{!open ? (
					<motion.button
						...
						className={cn(
							"flex size-full items-center rounded-xl bg-background/60 text-left backdrop-blur-md outline-none [--search-trigger-stroke:var(--color-border)] hover:[--search-trigger-stroke:color-mix(in_oklab,var(--color-foreground)_28%,transparent)] focus-visible:ring-2 focus-visible:ring-ring",
