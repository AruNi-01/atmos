...
export interface MorphPopoverContentProps {
  ...
  radius?: number;
  /** Clip-morph from the trigger corner. Off when the panel needs overflow. */
  clip?: boolean;
  className?: string;
}
    ...
    <AnimatePresence>
      {ctx.open ? (
        <motion.div
          ...
          className={cn(
            "fixed z-[9999]",
            clipEnabled && "[filter:drop-shadow(0_10px_18px_rgba(0,0,0,0.14))]",
          )}
        >
          <motion.div
            ref={ctx.contentRef}
            ...
            variants={clip}
            style={clipEnabled ? { borderRadius: radius } : undefined}
            className={cn(
              clipEnabled
