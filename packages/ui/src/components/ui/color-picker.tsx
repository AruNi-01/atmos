...
interface ColorPickerProps
  ...
  /**
   * Fired when the native EyeDropper session starts (`true`) or ends
   * (`false`, after pick or cancel). Host popovers/menus should keep
   * themselves open while this is true — focus leaves the panel so the
   * user can sample colors elsewhere on screen.
   */
  onEyedropperOpenChange?: (open: boolean) => void;
...
// Fallbacks keep the gradient valid even if theme tokens aren't injected yet.
// (An invalid layer in a multi-image background discards the whole property.)
const CHECKER_A = "var(--checker-a, #e0e0e0)";
const CHECKER_B = "var(--checker-b, #ffffff)";
const CHECKER_BG: CSSProperties = {
  backgroundImage: `conic-gradient(${CHECKER_A} 0 25%, ${CHECKER_B} 0 50%, ${CHECKER_A} 0 75%, ${CHECKER_B} 0)`,
...
interface SaturationSquareProps {
  ...
  /** Fired when a drag ends (pointer up) or a keyboard nudge commits. */
  onChangeEnd?: () => void;
}

function SaturationSquare({ h, s, v, onChange, onChangeEnd }: SaturationSquareProps) {
...
function AlphaSlider({
  ...
  // Checker layer sits behind so “empty” alpha reads as a checkerboard, not a void.
  const transparentColor = `rgba(${solidR}, ${solidG}, ${solidB}, 0)`;
  const opaqueColor = `rgb(${solidR}, ${solidG}, ${solidB})`;
  return (
    <Slider
      ...
      trackStyle={{
        backgroundImage: [
          `linear-gradient(to right, ${transparentColor} 0%, ${opaqueColor} 100%)`,
          `conic-gradient(${CHECKER_A} 0 25%, ${CHECKER_B} 0 50%, ${CHECKER_A} 0 75%, ${CHECKER_B} 0)`,
        ].join(", "),
...
/**
 * Ref-count of active native EyeDropper sessions. Host menus/popovers can
 * call `isColorEyedropperActive()` in their `onOpenChange` / dismiss handlers
 * so they stay mounted while the user samples colors elsewhere on screen.
 */
let colorEyedropperActiveCount = 0;

function isColorEyedropperActive(): boolean {
  return colorEyedropperActiveCount > 0;
}

function setColorEyedropperActive(active: boolean) {
  if (active) {
    colorEyedropperActiveCount += 1;
  } else {
    colorEyedropperActiveCount = Math.max(0, colorEyedropperActiveCount - 1);
  }
}
...
function EyeDropperButton({
  ...
  const handleClick = async (event: React.MouseEvent<HTMLButtonElement>) => {
    // Keep host popovers/menus from treating this as a dismiss gesture.
    event.preventDefault();
    event.stopPropagation();
    setColorEyedropperActive(true);
    onOpenChange?.(true);
    try {
...
function SwatchStrip({
  ...
  return (
    // py/px leave room for the selected/hover outer ring (box-shadow up to 4px)
    // so it isn't clipped by the panel or adjacent layout.
    <div className="flex flex-nowrap items-center justify-between gap-1.5 overflow-visible px-0.5 py-1">
      {swatches.map((sw, i) => {
...
const ColorPicker = forwardRef<HTMLDivElement, ColorPickerProps>(
  (
    ...
    // Keep a live ref so drag handlers can read the latest HSV without stale
    // closures, and so commit-on-pointer-up can emit without re-running setState.
    const hsvRef = useRef(initialParsed);
    hsvRef.current = hsv;
    ...
    useEffect(() => {
      ...
      // Also skip when the controlled value is an equivalent encoding of what we
      // just emitted (e.g. parent rewrites "#fff" → "rgba(255,255,255,1)").
      const emittedParsed = emitted ? parseColor(emitted) : null;
      const curParsed = parseColor(cur);
      if (
        emittedParsed &&
        curParsed &&
        Math.round(emittedParsed.r) === Math.round(curParsed.r) &&
        Math.round(emittedParsed.g) === Math.round(curParsed.g) &&
        Math.round(emittedParsed.b) === Math.round(curParsed.b) &&
        Math.abs(emittedParsed.a - curParsed.a) < 0.001
      ) {
    ...
    const emitValue = useCallback(
      (merged: { h: number; s: number; v: number; a: number }) => {
    ...
    /**
     * Update internal HSV.
     * - `emit: true` (default) notifies the parent immediately (inputs, swatches, eyedropper).
     * - `emit: false` only updates the panel UI during continuous drag; call
     *   `commitHsv` on pointer-up so parent state is not thrashed every frame.
     */
    const updateHsv = useCallback(
      (
        ...
        setHsv(merged);
        if (emit) emitValue(merged);
      },
      [emitValue]
    );

    const commitHsv = useCallback(() => {
      emitValue(hsvRef.current);
    }, [emitValue]);
    ...
    const handleHexCommit = useCallback(
      (input: string) => {
        ...
        const newHsv = rgbToHsv(p.r, p.g, p.b);
        const merged = {
          h: newHsv.s === 0 ? hsvRef.current.h : newHsv.h,
          s: newHsv.s,
          v: newHsv.v,
          a: p.a,
        };
    ...
    return (
      ...
      <div
        ref={ref}
        className={cn(
          // overflow-visible so swatch selected rings (outer box-shadow) aren't clipped
          "flex flex-col gap-2 overflow-visible p-3",
      ...
      >
        <SaturationSquare
          ...
          s={hsv.s}
          v={hsv.v}
          onChange={(s, v) => updateHsv({ s, v }, { emit: false })}
          onChangeEnd={commitHsv}
        />
        ...
        <div className="flex flex-col [&>*]:mb-0 [&>*+*]:-mt-px">
          ...
          <AlphaSlider
            ...
            solidG={solidG}
            solidB={solidB}
            onChange={(a) => updateHsv({ a }, { emit: false })}
            onChangeEnd={commitHsv}
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          {!hideEyedropper && (
            <EyeDropperButton
              onPick={handleEyedrop}
              onOpenChange={onEyedropperOpenChange}
            />
          )}
        </div>
...
const ColorPickerPopover = forwardRef<HTMLDivElement, ColorPickerPopoverProps>(
  (
    ...
    const handleOpenChange = useCallback(
      (next: boolean) => {
        // Stay open while the native eyedropper is active so the user can
        // sample colors outside the panel without losing the picker.
        if (!next && eyedroppingRef.current) return;
    ...
    const onEyedropperOpenChangeProp = pickerProps.onEyedropperOpenChange;
    const handleEyedropperOpenChange = useCallback(
      (active: boolean) => {
        eyedroppingRef.current = active;
        onEyedropperOpenChangeProp?.(active);
      },
      [onEyedropperOpenChangeProp]
    );
                  ...
                  <ColorPickerPortalContainer value={panelEl}>
                    <SurfaceProvider value={level}>
                      <ColorPicker
                        ...
                        onEyedropperOpenChange={handleEyedropperOpenChange}
                        className={cn(
