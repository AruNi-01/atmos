...
interface SliderProps
  ...
  /** Fired when a drag gesture ends (pointer up). */
...
const Slider = forwardRef<HTMLDivElement, SliderProps>(
  (
    ...
    const handlePointerUp = useCallback(() => {
      ...
      const tw = trackWidthRef.current;
      const thumbIndex = activeDragThumb.current;
      const motionX = thumbIndex === 0 ? motionX0 : motionX1;
    ...
    }, [min, max, step, stepValues, motionX0, motionX1, onChangeEnd, isRange, values]);
    ...
    const handleRadixChange = useCallback(
      (newValues: number[]) => {
        ...
        if (isRange) {
          ...
          onChangeEnd?.(mapped as [number, number]);
        } else {
          onChange(mapped[0]);
          onChangeEnd?.(mapped[0]);
        }
      },
      [isRange, onChange, onChangeEnd, stepValues]
    );
