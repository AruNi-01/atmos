...
export interface SliderOptions {
  ...
  /** Fires once when a pointer drag ends, with the snapped value. */
  onValueCommit?: (value: number) => void;
  /** Pointer position 0–100 while dragging; does not snap. */
...
export function useSlider({
  ...
  const onDragPercentRef = useRef(onDragPercent);
  onDragPercentRef.current = onDragPercent;

  const commit = useCallback(
    (next: number) => {
      const clean = snapSliderValue(next, lo, hi, stride);
      if (clean === currentRef.current) return clean;
      currentRef.current = clean;
      if (!controlled) setInternal(clean);
      onValueChangeRef.current?.(clean);
      return clean;
    },
    [controlled, lo, hi, stride],
  );
  ...
  const commitFromX = useCallback(
    (clientX: number) => {
      ...
      onDragPercentRef.current?.(ratio * 100);
  ...
  const endDrag = useCallback((event: PointerEvent<HTMLDivElement>) => {
    ...
    setDragging(false);
    onValueCommitRef.current?.(currentRef.current);
  }, []);
